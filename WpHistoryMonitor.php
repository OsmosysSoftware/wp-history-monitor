...
class WpHistoryMonitor {
    ...
    private function enqueueStyles() {
        wp_register_style('bootstrap-css', plugins_url() . '/wp-history-monitor/css/bootstrap.min.css');
        wp_enqueue_style('bootstrap-css');
        wp_register_style('bootstrap-datepicker-css', plugins_url() . '/wp-history-monitor/css/datepicker-min.css');
        wp_enqueue_style('bootstrap-datepicker-css');
        wp_register_style('pnotify.custom.min', plugins_url() . '/wp-history-monitor/css/pnotify.custom.min.css');
        wp_enqueue_style('pnotify.custom.min');
        wp_register_style('datatable-css', plugins_url() . '/wp-history-monitor/css/datatables.min.css');
        wp_enqueue_style('datatable-css');
        wp_enqueue_style('style', plugins_url() . '/wp-history-monitor/css/style.css');
    }

    // Function to enqueue all the scripts.
    private function enqueueScripts() {
        wp_enqueue_script('jquery');
        wp_register_script('bootstrap-js', plugins_url() . '/wp-history-monitor/js/bootstrap.min.js');
        wp_enqueue_script('bootstrap-js');
        wp_register_script('moment-js', plugins_url() . '/wp-history-monitor/js/moment.js');
        wp_enqueue_script('moment-js');
        wp_register_script('pnotify.custom.min', plugins_url() . '/wp-history-monitor/js/pnotify.custom.min.js');
        wp_enqueue_script('pnotify.custom.min');
        wp_register_script('bootstrap-datepicker-script', plugins_url() . '/wp-history-monitor/js/datepicker.js');
        wp_enqueue_script('bootstrap-datepicker-script');
        wp_register_script('datatable-js', plugins_url() . '/wp-history-monitor/js/datatables.min.js');
        wp_enqueue_script('datatable-js');
        wp_enqueue_script('blockui', plugins_url() . '/wp-history-monitor/js/blockui.js');
        wp_enqueue_script('script', plugins_url() . '/wp-history-monitor/js/script.js');
        wp_localize_script('script', 'MyAjax', array('ajaxurl' => admin_url('admin-ajax.php')));
    }

    // Function to add menu page to wordpress dashboard. 
    public function showUserDataUpToRequiredDate() {
        add_menu_page('Activity', 'History Monitor', 'activate_plugins', 'userpage', array($this, 'WpHistoryMonitor'), plugins_url('wp-history-monitor/images/history-monitor-image.png'), 6);
    }
